...
//        leetCodeMain17.lengthOfLongestSubstring("abcabcbb");
    ...
    // 这里是全排列，所以说必须扫描完数组中所有的内容，才可以返回
    public void permuteDfs(int[] nums, int len, int depth, List<Integer> path, boolean[] used, List<List<Integer>> res) {
        if (depth == len) {
            res.add(path);
            return;
        }
